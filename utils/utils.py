...
def is_ext_file(filename: str, extensions=IMG_EXTENSIONS):
    return any(filename.endswith(extension) for extension in extensions)


def scan_dir(path: Path, extensions=IMG_EXTENSIONS) -> List[Path]:
    if not path.is_dir():
        raise AssertionError(f'"{path.absolute()}" is not a valid directory.')
    files_list: List[Path] = []
    for ext in extensions:
        files_list.extend(path.glob(f"**/*{ext}"))
    files_list = sorted(list(files_list))
    return files_list


def get_models_paths(path: Path) -> List[Path]:
    """Get model path list from model folder"""
    models = scan_dir(path, MODEL_EXTENSIONS)
    if not models:
        raise AssertionError(f'"{path.absolute()}" has no valid model file.')
    return models


def get_images_paths(path: Path) -> List[Path]:
    """Get image path list from image folder"""
    images = scan_dir(path, IMG_EXTENSIONS)
    if not images:
        raise AssertionError(f'"{path.absolute()}" has no valid image file.')
    return images
...
def read_img(path: Path) -> np.ndarray:
    ...
    img = None
    if path:
        if rawpy_available and path.suffix.lower() == ".dng":
            # if image is a DNG
            with rawpy.imread(str(path.absolute())) as raw:
                img = raw.postprocess()
        else:
